...
export default function GroupsPage() {
  ...
  const [formData, setFormData] = React.useState({ name: '', description: '', color: COLORS[0] });
  ...
  const openEdit = (group: Group) => {
    ...
    setFormData({ name: group?.name || '', description: group?.description || '', color: group?.color || COLORS[0] });
    setFormError('');
    setShowModal(true);
  };
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {groups?.map?.((group) => (
              <Card key={group?.id} className="hover:shadow-lg transition-all cursor-pointer group">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Users className="h-5 w-5" style={{ color: group?.color || '#3B82F6' }} />
                      </div>
                      <div>
